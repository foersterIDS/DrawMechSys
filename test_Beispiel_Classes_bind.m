...
% Spuren
trails{1} = gfx2d.trail(Ntr1,[],[],0.03,'Color','m','FaceAlpha',fa1,'Wind',[0;0.03]); % Slider
trails{2} = gfx2d.trail(Ntr2,[],[],0.08,'Color','m','FaceAlpha',fa2); % Sphere
% Slider:
...
bearings{2} = gfx2d.floatingbearing(8,8-rr,bl,8,8-rr,[0;1],'npl',npl,'wallsize',[-0.75,0.75],'Color',co,'LineWidth',lw);
bearings{3} = gfx2d.floatingbearing(12,8-rr,bl,12,8-rr,[0;1],'npl',npl,'wallsize',[-1,+1],'Color',co,'LineWidth',lw);
% Kraefte & Momente:
...
%% bind springs
springs{1}.bind(masses{1},[1,0.75],true);
springs{1}.bind(sliders{1},[1,1],false);
springs{2}.bind(masses{1},[-1,0.5],false);
springs{3}.bind(masses{1},[1,0],true);
springs{3}.bind(masses{2},[0,0],false);
springs{4}.bind(masses{2},[0,0],true);
springs{4}.bind(joints{5},[0,0],false);
springs{5}.bind(masses{1},[0,0],true);
springs{5}.bind(joints{3},[0,0],false);
springs{6}.bind(masses{3},[0,0],true);
springs{6}.bind(bearings{3},[0,0],false);

nlsprings{1}.bind(masses{1},[-1,0],false);
nlsprings{2}.bind(joints{6},[0,0],true);
nlsprings{2}.bind(masses{3},[0,0],false);

%% bind dampers
dampers{1}.bind(masses{1},[-1,-0.5],false);
dampers{2}.bind(hulls{2},[1,0],true);
dampers{2}.bind(joints{3},[0,0],false);

%% bind H-shape
masses{5}.bind(bars{3},[1,0],[0,0.5],true,pi/2); %% todo: offset

%% bind Joints
joints{1}.bind(masses{1},[0,0]);
joints{2}.bind(masses{1},[1,0]);
joints{3}.bind(masses{4},[0,0]);
joints{4}.bind(bars{1},[0,0]);
joints{5}.bind(masses{3},[1,pi/2]);
joints{6}.bind(hulls{1},[1 0]);
...
for i=1:N%[1:N,1:N,1:N]%
    ...
    masses{3}.setPosition(8+q4,8-rr,q3);
